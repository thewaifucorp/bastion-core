...
Persona contract v2's `tools:` field (`bastion-personas`'s `PersonaFront`/`Persona`) is an allowlist, not documentation. `CapabilityRegistry::invoke`'s Policy 0 denies any capability name outside the dispatching persona's resolved `allowed_tools` set BEFORE the egress/approval policies run. Two bypass paths have no `Capability`/`InvokeCtx` of their own and apply the identical `check_tool_allowed` gate directly, inline, before dispatch — the same historical blind spot invariant #2's egress check needed extra call sites for: the empty-registry MCP-bypass path in `dispatch_tool_loop`, and `run_provider_fallback` (reached when persona routing produced empty `route_text`, which can still mean a real persona is attributed to the turn — `RespondOutcome.allowed_tools`, resolved by the `Responder` the same way as `RespondOutcome.turn_tier`, threads the gate across the kernel/persona-crate port boundary into this path). `allowed_tools: None` (absent `tools:`, or no persona resolved) stays unrestricted — this is the back-compat default for every pre-contract-v2 SOUL.md, never a silent deny-all.

- **Code:** `crates/bastion-runtime/src/capability/registry.rs` (`InvokeCtx::allowed_tools`, `check_tool_allowed`, Policy 0), `crates/bastion-runtime/src/agent/loop_.rs` (`dispatch_tool_loop`'s registry path and empty-registry bypass, `run_provider_fallback`), `crates/bastion-runtime/src/agent/ports.rs` (`RespondOutcome::allowed_tools`), `crates/bastion-personas/src/persona/responder.rs` (`resolve_allowed_tools`), `crates/bastion-personas/src/persona/soul.rs` (`PersonaFront::tools`, `validate()`).
- **Evidence:** `invoke_denies_tool_not_in_allowed_set`, `invoke_allows_any_tool_when_allowed_tools_is_none`, `invoke_allows_tool_present_in_allowed_set`, and the `check_tool_allowed_*` unit tests in `registry.rs`; new coverage for the fallback path tracked alongside this change.
